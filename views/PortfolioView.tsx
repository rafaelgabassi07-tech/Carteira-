...
const Header: React.FC<{ 
    ...
    isAnalyzing?: boolean;
}> = ({ setActiveView, onShare, onRefresh, isRefreshing, isAnalyzing }) => {
    const { t } = useI18n();
        ...
        <header className="px-4 py-3 flex justify-between items-center sticky top-0 z-30 glass border-b border-[var(--border-color)] transition-all duration-300">
            ...
            <div className="flex items-center space-x-2">
                <button 
                    ...
                    className={`p-2 rounded-full bg-[var(--bg-secondary)] hover:bg-[var(--bg-tertiary-hover)] text-[var(--text-secondary)] transition-all active:scale-95 ${isRefreshing ? 'animate-spin text-[var(--accent-color)]' : ''} ${isAnalyzing && !isRefreshing ? 'text-[var(--accent-color)]' : ''}`} 
                    aria-label={t('refresh_prices')}
                >
                     <RefreshIcon className={`w-5 h-5 ${isAnalyzing && !isRefreshing ? 'animate-pulse' : ''}`}/>
                </button>
...
const PortfolioView: React.FC<PortfolioViewProps> = ({ setActiveView, onSelectAsset, addToast }) => {
    const { t, formatCurrency } = useI18n();
    const { assets, refreshMarketData, privacyMode, preferences, isRefreshing: isContextRefreshing, isAnalyzing } = usePortfolio();
    ...
    return (
        ...
        >
            ...
            <div className="max-w-7xl mx-auto">
                <Header setActiveView={setActiveView} onShare={handleShare} onRefresh={handleRefreshPrices} isRefreshing={isRefreshing} isAnalyzing={isAnalyzing} />
