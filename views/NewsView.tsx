...
const SentimentIndicator: React.FC<{ score?: number; reason?: string }> = ({ score, reason }) => {
    const { t } = useI18n();
    
    if (score === undefined) return null;

    const getSentimentDetails = (s: number) => {
        if (s > 0.6) return { label: t('sentiment_very_positive'), color: 'text-emerald-400', gradient: 'from-green-500 to-emerald-500' };
        if (s > 0.2) return { label: t('sentiment_positive'), color: 'text-green-400', gradient: 'from-yellow-500 to-green-500' };
        if (s > -0.2) return { label: t('sentiment_neutral'), color: 'text-gray-400', gradient: 'from-gray-500 to-gray-500' };
        if (s > -0.6) return { label: t('sentiment_negative'), color: 'text-rose-400', gradient: 'from-orange-500 to-rose-500' };
        return { label: t('sentiment_very_negative'), color: 'text-red-400', gradient: 'from-rose-500 to-red-500' };
    };

    const { label, color, gradient } = getSentimentDetails(score);
    const widthPercentage = ((score + 1) / 2) * 100;

    const content = (
        <div className="flex items-center gap-2 cursor-help">
            <div className="flex items-center gap-1.5 group">
                <SparklesIcon className="w-3 h-3 text-sky-400/70" />
                <div className="w-10 h-1 bg-[var(--bg-primary)] rounded-full overflow-hidden border border-[var(--border-color)]">
                    <div className={`h-full rounded-full bg-gradient-to-r ${gradient}`} style={{ width: `${widthPercentage}%` }} />
                </div>
            </div>
    ...
    >
        ...
        <div className="p-5 flex flex-col flex-1">
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t border-[var(--border-color)]">
                ...
                <SentimentIndicator score={article.sentimentScore} reason={article.sentimentReason} />
            </div>
        </div>
    </a>
  );
