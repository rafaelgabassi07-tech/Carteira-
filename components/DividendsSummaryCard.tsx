...
const DividendsSummaryCard: React.FC<DividendsSummaryCardProps> = ({ setActiveView }) => {
    ...
    const { averageIncome, chartData } = useMemo(() => {
        ...
        // Ensure we have at least some height for bars even if value is 0 relative to max
        const maxVal = Math.max(...last6Months.map(d => d.total), 1);

        const chart = last6Months.map((d, index) => ({
            month: d.month,
            value: d.total,
            heightPercent: (d.total / maxVal) * 100,
    ...
    return (
        <div 
            ...
            className="group relative overflow-hidden rounded-[24px] bg-[var(--bg-secondary)] border border-[var(--border-color)] p-0 cursor-pointer transition-all duration-300 hover:shadow-2xl hover:border-[var(--accent-color)]/40 hover:-translate-y-1 active:scale-[0.99] w-full"
        >
            {/* Gradient Background Effect */}
            <div className="absolute inset-0 bg-gradient-to-b from-transparent to-[var(--bg-tertiary-hover)]/50 opacity-50 pointer-events-none"></div>
            ...
            {/* Chart Area - Full Width Bottom */}
            <div className="relative h-24 w-full mt-2 px-2 pb-0">
                <div className="flex items-end justify-between h-full gap-2 px-4 pb-0">
                    {chartData.map((d, i) => (
                        <div key={i} className="flex-1 flex flex-col justify-end items-center h-full group/bar relative pb-3">
                            {/* Bar Value Tooltip */}
                            <div className="absolute bottom-full mb-1 opacity-0 group-hover/bar:opacity-100 transition-opacity duration-200 pointer-events-none bg-[var(--text-primary)] text-[var(--bg-primary)] text-[9px] font-bold px-1.5 py-0.5 rounded shadow-lg whitespace-nowrap z-20">
                                {formatCurrency(d.value)}
                            </div>

                            {/* The Bar */}
                            <div 
                                className="w-full max-w-[20px] rounded-t-sm relative transition-all duration-500 ease-out group-hover/bar:brightness-110"
                                style={{ 
                                    height: `${Math.max(d.heightPercent, 12)}%`, 
